...
const FrogRoad: React.FC<ChickenProps> = ({ onBack, onResult }) => {
    ...
    let translateX = 0;
    // We want to center the current lane but show the NEXT one clearly.
    // If we shift -25vw per lane, the current lane is always at the left-most edge (if starting at 0).
    // We want current lane to be slightly left of center to allow seeing right.
    if (currentLane > -1) {
        translateX = -(currentLane + 1) * 25 + 25; // Keep current lane in view 2
    }
        ...
        <div className="bg-[#1a1c20] min-h-screen flex flex-col font-sans text-white overflow-hidden">
            ...
            <div className="bg-[#24262b] p-3 flex items-center justify-between border-b border-gray-800 z-20">
                ...
                <div className="flex items-center gap-3">
                    <div className="bg-black/30 px-3 py-1 rounded-full border border-gray-700 text-sm font-mono text-green-400">
            ...
            <div className="flex-1 relative overflow-hidden bg-[#2b2e33]">
                <div 
                    ...
                    style={{ 
                        transform: `translateX(${translateX + 10}vw)`, // Shift +10vw to show right side better
                ...
                >
                    ...
                    <div className="w-[25vw] h-full flex-shrink-0 bg-[#3a4042] border-r-4 border-gray-500 relative flex flex-col justify-center items-center z-10">
                        ...
                        <div className="w-20 h-20 bg-green-800/20 rounded-full flex items-center justify-center mb-8 border-2 border-green-900/50">
                        ...
                        {currentLane === -1 && gameState !== 'CRASHED' && gameState !== 'CRASHING' && (
                            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-20 transition-all duration-300">
                                <img src="https://cdn-icons-png.flaticon.com/512/616/616408.png" alt="Frog" className="w-16 h-16 object-contain drop-shadow-2xl animate-bounce" />
                    ...
                    {MULTIPLIERS[difficulty].map((mult, index) => {
                        ...
                        const isFrogHere = currentLane === index;
                        ...
                        return (
                            <div key={index} className={`w-[25vw] h-full flex-shrink-0 relative border-r-2 border-dashed border-gray-600/50 bg-[#2b2e33] flex items-center justify-center ${isNext ? 'brightness-110' : ''}`}>
                                ...
                                {/* Frog */}
                                {isFrogHere && gameState !== 'CRASHED' && gameState !== 'CRASHING' && (
                                    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-20 animate-in zoom-in duration-300">
                                        ...
                                        <div className="absolute -top-8 left-1/2 -translate-x-1/2 bg-green-600 text-white text-[10px] font-bold px-2 py-0.5 rounded shadow-lg whitespace-nowrap">
                                            {mult}x
                                        </div>
                                    </div>
                                )}
                                ...
                                {isCrashSite && (
                                    <>
                                        {/* Frog getting hit state */}
                                        {gameState === 'CRASHING' && (
                                           <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-20">
                                               <img src="https://cdn-icons-png.flaticon.com/512/616/616408.png" alt="Frog" className="w-16 h-16 object-contain grayscale opacity-50" />
                                           </div>
                                        )}
