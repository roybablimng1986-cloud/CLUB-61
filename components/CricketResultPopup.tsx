...
import { X, Trophy, AlertCircle, Trophy as Cup } from 'lucide-react';
import { playSound } from '../services/supabaseService';

interface CricketResultPopupProps {
  result: {
    win: boolean;
    amount: number;
    period: string;
...
const CricketResultPopup: React.FC<CricketResultPopupProps> = ({ result, onClose }) => {
  React.useEffect(() => {
    if (result) {
  ...
  const getOutcomeLabel = (val: number) => {
    if (val === 0) return 'WICKET';
    return `${val} RUNS`;
  };
  ...
  const getOutcomeColor = (val: number) => {
    ...
    if (val === 4) return 'text-purple-500';
          ...
          <div className={`p-8 text-center relative ${result.win ? 'bg-gradient-to-b from-green-500/20 to-transparent' : 'bg-gradient-to-b from-red-500/20 to-transparent'}`}>
            ...
            <div className="space-y-4 mb-8">
                <div className="bg-black/30 p-6 rounded-3xl border border-white/5">
                    <p className="text-[8px] font-black text-white/30 uppercase tracking-widest mb-4">Match Result</p>
                    <div className="flex flex-col items-center">
                        <p className={`text-6xl font-black italic tracking-tighter ${getOutcomeColor(result.landed)}`}>
                            {result.landed === 0 ? 'W' : result.landed}
                        </p>
                        <p className="text-xl font-black italic gold-text uppercase mt-2">{getOutcomeLabel(result.landed)}</p>
                    </div>
                </div>
                ...
                <div className="bg-black/30 p-4 rounded-3xl border border-white/5 flex justify-between items-center">
                    <div>
                        ...
                        <p className="text-xl font-black text-white uppercase">{getOutcomeLabel(result.target)}</p>
                    </div>
                </div>
            </div>
...
export default CricketResultPopup;
