...
interface JhandiMundaResultPopupProps {
  result: {
    win: boolean;
    amount: number;
    period: string;
...
const JhandiMundaResultPopup: React.FC<JhandiMundaResultPopupProps> = ({ result, onClose }) => {
  React.useEffect(() => {
    if (result) {
            ...
            <div className="space-y-4 mb-8">
                <div className="bg-black/30 p-6 rounded-3xl border border-white/5">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                        {result.dice.map((d, i) => (
                            <div key={i} className="w-full aspect-square bg-white rounded-xl flex items-center justify-center text-2xl shadow-inner border-b-2 border-slate-300">
                                {d}
                            </div>
                        ))}
                    </div>
                ...
                <div className="bg-black/30 p-4 rounded-3xl border border-white/5 flex justify-between items-center">
                    <div>
                        <p className="text-[8px] font-black text-white/30 uppercase tracking-widest mb-1">Your Symbol</p>
                        <p className="text-2xl font-black text-white uppercase">{result.target}</p>
                    </div>
                    <div className="text-right">
                        <p className="text-[8px] font-black text-white/30 uppercase tracking-widest mb-1">Matches</p>
                        <p className="text-2xl font-black text-yellow-500 uppercase">{result.matches}</p>
                    </div>
                </div>
            </div>
...
export default JhandiMundaResultPopup;
