...
import { X, Trophy, AlertCircle, ChevronUp, ChevronDown } from 'lucide-react';
        ...
        >
          <div className={`p-8 text-center relative ${result.win ? 'bg-gradient-to-b from-yellow-500/20 to-transparent' : 'bg-gradient-to-b from-red-500/20 to-transparent'}`}>
            ...
            <h2 className={`text-4xl font-black italic tracking-tighter uppercase mb-2 ${result.win ? 'text-yellow-500' : 'text-red-500'}`}>
              {result.win ? 'ROYALE WIN!' : 'GAME OVER'}
            </h2>

            <div className="space-y-4 mb-8">
                <div className="bg-black/30 p-6 rounded-3xl border border-white/5">
                    <p className="text-[8px] font-black text-white/30 uppercase tracking-widest mb-4">Final Card</p>
                    <div className="flex flex-col items-center">
                        <div className="w-20 h-28 bg-white rounded-xl flex flex-col items-center justify-center border-2 border-slate-300">
                            <span className={`text-3xl font-black ${result.finalCard.suit === '♥' || result.finalCard.suit === '♦' ? 'text-red-600' : 'text-black'}`}>{result.finalCard.rank}</span>
                            <span className={`text-2xl ${result.finalCard.suit === '♥' || result.finalCard.suit === '♦' ? 'text-red-600' : 'text-black'}`}>{result.finalCard.suit}</span>
                        </div>
                        {result.win && (
                            <p className="text-xl font-black italic gold-text uppercase mt-4">{result.multiplier.toFixed(2)}x Multiplier!</p>
                        )}
                    </div>
                </div>
            </div>
