...
interface CyberBurstResultPopupProps {
  result: {
    win: boolean;
    amount: number;
    multiplier: number;
...
const CyberBurstResultPopup: React.FC<CyberBurstResultPopupProps> = ({ result, onClose }) => {
  React.useEffect(() => {
    if (result) {
        ...
        >
          <div className={`p-8 text-center relative ${result.win ? 'bg-gradient-to-b from-cyan-500/20 to-transparent' : 'bg-gradient-to-b from-red-500/20 to-transparent'}`}>
            ...
            <h2 className={`text-4xl font-black italic tracking-tighter uppercase mb-2 ${result.win ? 'text-cyan-500' : 'text-red-500'}`}>
              {result.win ? 'CAPTURED!' : 'BURST!'}
            </h2>

            <div className="space-y-4 mb-8">
                <div className="bg-black/30 p-6 rounded-3xl border border-white/5">
                    <p className="text-[8px] font-black text-white/30 uppercase tracking-widest mb-4">Energy Level</p>
...
export default CyberBurstResultPopup;
