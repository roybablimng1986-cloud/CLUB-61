...
interface SicBoResultPopupProps {
  result: {
    win: boolean;
    amount: number;
    period: string;
...
const SicBoResultPopup: React.FC<SicBoResultPopupProps> = ({ result, onClose }) => {
  React.useEffect(() => {
    if (result) {
          ...
          <div className={`p-8 text-center relative ${result.win ? 'bg-gradient-to-b from-green-500/20 to-transparent' : 'bg-gradient-to-b from-red-500/20 to-transparent'}`}>
            ...
            <div className="space-y-4 mb-8">
                <div className="bg-black/30 p-6 rounded-3xl border border-white/5">
                    <p className="text-[8px] font-black text-white/30 uppercase tracking-widest mb-4">Result Dice</p>
                    <div className="flex justify-center gap-4">
                        {result.dice.map((d, i) => (
                            <div key={i} className="text-5xl drop-shadow-[0_4px_10px_rgba(0,0,0,1)] text-white">
                                {DICE_FACES[d-1]}
                            </div>
                        ))}
                    </div>
                    <div className="mt-4 flex justify-center gap-4">
                        <div className="px-3 py-1 bg-white/5 rounded-full text-[10px] font-black uppercase text-yellow-500">Sum: {result.sum}</div>
                        <div className="px-3 py-1 bg-white/5 rounded-full text-[10px] font-black uppercase text-yellow-500">{result.sum >= 11 ? 'Big' : 'Small'}</div>
                    </div>
                </div>
...
export default SicBoResultPopup;
