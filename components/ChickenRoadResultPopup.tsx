...
interface ChickenRoadResultPopupProps {
  result: {
    win: boolean;
    amount: number;
    multiplier: number;
...
const ChickenRoadResultPopup: React.FC<ChickenRoadResultPopupProps> = ({ result, onClose }) => {
  React.useEffect(() => {
    if (result) {
        ...
        >
          <div className={`p-8 text-center relative ${result.win ? 'bg-gradient-to-b from-green-500/20 to-transparent' : 'bg-gradient-to-b from-red-500/20 to-transparent'}`}>
            ...
            <h2 className={`text-4xl font-black italic tracking-tighter uppercase mb-2 ${result.win ? 'text-green-500' : 'text-red-500'}`}>
              {result.win ? 'ROAD MASTER!' : 'ROAD KILL'}
            </h2>

            <div className="space-y-4 mb-8">
                <div className="bg-black/30 p-6 rounded-3xl border border-white/5">
                    <p className="text-[8px] font-black text-white/30 uppercase tracking-widest mb-4">Journey Stats</p>
                    <div className="flex flex-col items-center">
                        <p className={`text-xl font-black italic uppercase ${result.win ? 'text-green-400' : 'text-red-500'}`}>
                            Lane {result.lane + 1} Reached
                        </p>
                        <p className="text-sm text-white/50 uppercase tracking-widest mt-1">{result.difficulty} Mode</p>
                        {result.win && (
...
export default ChickenRoadResultPopup;
