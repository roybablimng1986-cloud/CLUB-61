...
import { X, Trophy, AlertCircle, Car } from 'lucide-react';
import { playSound } from '../services/supabaseService';

interface CarRouletteResultPopupProps {
  result: {
    win: boolean;
    amount: number;
    period: string;
    winner: { id: string; icon: string; label: string; mult: number };
    target: string;
  } | null;
  onClose: () => void;
}

const CarRouletteResultPopup: React.FC<CarRouletteResultPopupProps> = ({ result, onClose }) => {
  React.useEffect(() => {
    if (result) {
          ...
          <div className={`p-8 text-center relative ${result.win ? 'bg-gradient-to-b from-green-500/20 to-transparent' : 'bg-gradient-to-b from-red-500/20 to-transparent'}`}>
            ...
            <div className="mb-6 inline-flex p-6 rounded-full bg-black/40 border border-white/10 relative">
              {result.win ? (
                ...
                <Car size={48} className="text-red-500 animate-pulse" />
              )}
            </div>
            ...
            <div className="space-y-4 mb-8">
                <div className="bg-black/30 p-6 rounded-3xl border border-white/5">
                    <p className="text-[8px] font-black text-white/30 uppercase tracking-widest mb-4">Winning Brand</p>
                    <div className="flex flex-col items-center">
                        <div className="w-24 h-24 bg-zinc-800 rounded-2xl p-4 border border-white/10 shadow-2xl mb-2">
                            <img src={result.winner.icon} alt={result.winner.label} className="w-full h-full object-contain" referrerPolicy="no-referrer" />
                        </div>
                        <p className="text-xl font-black italic gold-text uppercase">{result.winner.label}</p>
                        <p className="text-[10px] font-black text-yellow-500/60 uppercase tracking-widest">{result.winner.mult}X Payout</p>
                    </div>
                </div>
...
export default CarRouletteResultPopup;
