...
interface NavBarProps { currentView: View; setView: (view: View) => void; }
    ...
    <div className="fixed bottom-0 left-0 w-full bg-[#1e293b] border-t border-slate-700 pb-safe z-50">
      <div className="flex justify-around items-center h-16">
        {navItems.map((item) => {
          ...
          return (
            <button key={item.id} onClick={() => setView(item.id as View)} className={`flex flex-col items-center justify-center w-full h-full transition-colors ${isActive ? 'text-blue-400' : 'text-slate-400 hover:text-slate-200'}`}>
